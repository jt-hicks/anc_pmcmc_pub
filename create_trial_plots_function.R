create_dashboard_plots_trial <- function(results,
  ...
  smc_times <- data.frame(country=factor(c(rep('Ghana',8),rep('Burkina Faso',8),rep('Mali',6),rep('Gambia',3)),
                                         levels=c('Ghana', 'Burkina Faso', 'Mali', 'Gambia',"Malawi","Kenya")),
  ...
  colors <-c(viridis::viridis(6,begin=0,end=0.95))
  names(colors) <- c('Ghana','Burkina Faso', 'Mali', 'Gambia','Malawi','Kenya')
  ...
  results_summary$country <- factor(results_summary$country,levels=c('Ghana', 'Burkina Faso', 'Mali', 'Gambia',"Malawi","Kenya"))
  results_sample$country <- factor(results_sample$country,levels=c('Ghana', 'Burkina Faso', 'Mali', 'Gambia',"Malawi","Kenya"))
  observed_cis$country <- factor(observed_cis$country,levels=c('Ghana', 'Burkina Faso', 'Mali', 'Gambia',"Malawi","Kenya"))
  y_axis_label <- NULL
  if(var=='prev_anc'){
  ...
  if(!is.null(rainfall)){
    ...
    rainfall$country <- factor(rainfall$country,levels=c('Ghana', 'Burkina Faso', 'Mali', 'Gambia',"Malawi","Kenya"))
